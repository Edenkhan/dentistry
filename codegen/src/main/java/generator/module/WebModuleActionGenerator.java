...
public class WebModuleActionGenerator implements ModuleGenerator {
    ...
    public WebModuleActionGenerator(JCodeModel cm) {
        this.cm = cm;
    ...
    public void _generate(Entities entities) throws Exception {
        ...
        String actionName = StringUtils.capitalize(moduleName) + "ModuleAction";
        ...
        clazz.annotate(cm.ref(actionAnnonationClassName)).param("alias", moduleName);
        clazz.annotate(permissionGroupClass).param("value", moduleName);
    ...
    private void generateDoMethod(JDefinedClass clazz, Entity entity, String moduleName) {
        generateDoPageMethod(clazz, entity, moduleName);
        generateDoDataMethod(clazz, entity, moduleName);
    }

    private void generateDoPageMethod(JDefinedClass clazz, Entity entity, String moduleName) {
        String methodName = getDoName(entity.getAlias() + "sPage");
        JMethod method = clazz.method(JMod.PUBLIC, pageClass, methodName);
        method.annotate(needPermissionClass).param("value", "访问" + entity.getDescription() + "页面");
        method.body()._return(pageFacotyClass.staticInvoke("createTablePage").arg(JExpr.lit(String.format("/%s/%ss", moduleName, entity.getAlias().replaceAll("_", "")))));
    }
    ...
    private void generateDoDataMethod(JDefinedClass clazz, Entity entity, String moduleName) {
        ...
        method.annotate(needPermissionClass).param("value", "获取" + entity.getDescription() + "数据");
...
//        body.decl(cm.ref(Pagination.class).narrow(cm.ref(Project.getInstance().getValueObjectClassName(entity))), "pagination")
//                .init(JExpr.ref(StringUtils.uncapitalize(serviceInterfaceClass.name())).invoke("query" + entity.getName() + "s").arg(JExpr.ref("qo")));
        body.decl(returnClass, "ret", JExpr.ref("returnFactory").invoke("createNormalReturn").arg(JExpr.ref("pagination")));
        body._return(pageFacotyClass.staticInvoke("createJsonPage").arg(JExpr.ref("ret")));
    }
